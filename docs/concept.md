...
| trait 群 | `StorageProvider` / `EmbeddingProvider` / `Retriever` / `RelevanceScorer` / `EventDetector` / `QueryClassifier` / `PromptCompressor` / `Summarizer` |
...
8 種の trait で主要な差し替え点を提供する。詳細と配置は `tech-architecture.md` を参照。
...
| `EventDetector` | 話題切替 / トリガー検知 | Tier 0-2 (cascade) |
| `QueryClassifier` | クエリ種別判定 (regex / SetFit) | Tier 0-1 |
| `PromptCompressor` | プロンプト圧縮 (LLMLingua-2 等) | Tier 2 |
| `Summarizer` | 階層的要約生成 (DistilBART 等) | Tier 2 |

### 4. 3-tier 処理階層 (encoder-only stack)

コンテキスト管理の各処理を 3 段階の計算コスト層に分けて実装する。**tsumugi は autoregressive LLM (Qwen / GPT / Claude 等) を一切呼ばない encoder-only スタック**。LLM 委譲版コンポーネント (旧 `LlmSummarizer` / `LlmDelegationCompressor` / `BertClassifier` (LLM-routed) / `LLMClassifierDetector`) は 2026-04 にすべて撤去された。

| Tier | 粒度 | コスト目安 | 例 |
|---|---|---|---|
| Tier 0 | 決定論 | μs 〜 ms | 正規表現、完全一致、BM25 (SQLite FTS5 + lindera)、階層走査、時間窓 |
| Tier 1 | CPU 軽量 encoder | 数 ms | 小型 embedding (MiniLM-L6 / e5-small / BGE-small)、SetFit query 分類器、IKE 二値化 |
| Tier 2 | CPU 中量 encoder | 数十 ms 〜 数秒 | LLMLingua-2 token-level 圧縮、mDeBERTa-XNLI zero-shot 検出、DistilBART abstractive 要約 |

帰結として、最小構成ユーザー (Apple Silicon 8GB / 統合 GPU 環境) でも tsumugi の全機能が CPU で動作する。GPU は速度向上に使われるが、**機能の有無を分けない**。LLM API への依存も無いので、ローカル完結 / オフライン運用 / プライバシー重視ユースケースで tsumugi 単体が完結する。
...
- **完全 encoder-only**: tsumugi 内部に autoregressive LLM 呼び出し無し。Tier 2 までで全機能が完結
- **日本語対応前提**: lindera による BM25、多言語 encoder (mDeBERTa-XNLI / multilingual-MiniLM) の swap が 1 行
...
| **SummaryMethod enum** | `LlmLingua2` / `SelectiveContext` / `ExtractiveBM25` / `DistilBart` / `UserManual` | 要約手法の選択を明示、Tier と直接対応。`LlmFull` は LLM 削除 (2026-04) で撤去、`DistilBart` を encoder-decoder abstractive 要約として追加 |
| **SourceLocation の表現** | trait 化 (core に `FileSourceLocation` 標準実装を同梱) | ファイルパス / URI / "session/3#scene2" 等の多様な表現をアプリケーションが差し替え可能 |
| **3-tier 処理階層** | 設計原則 #4 に明文化、全 Tier が encoder-only で完結 | autoregressive LLM 委譲を全削除 (2026-04)、ローカル / オフライン運用が標準 |
