...
2. **LLMLingua-2-mBERT 実装** (`PromptCompressor`) — **完了 (2026-04-30、
   sliding-window fix を 2026-04-30 に follow-up)**
   - `tsumugi-core/src/compressor/llm_lingua_v2.rs` 新設、
     `LlmLingua2Compressor` を `onnx` feature 配下で実装。per-token binary
     classifier (`[batch, seq, 2]` → softmax → keep_prob)、原文 word ratio
     を subword keep ratio に変換して top-K 選択、`preserve_tail_tokens`
     分の suffix subword は force-keep。WordPiece `##` continuation 判定で
     word boundary を追跡。デコードは `tokenizer.decode(skip_special=true)`
   - **長文入力の sliding-window 対応**: BERT-base は
     `max_position_embeddings=512` で頭打ち。tokenizer truncation を load
     時に切り、`max_seq - 2` ごとに content tokens を chunk して各 chunk
     先頭/末尾に [CLS]/[SEP] を付与し forward。session lock は loop 全体で
     1 回取得して O(N/512) 回の `Session::run` に均し、各 chunk の
     keep_prob を原座標に書き戻す。初回 cr dispatch (PR #33) で
     `compressed_chars / retrieval_chars ≈ 3.4%` の異常圧縮が出たのは、
     truncation で先頭 ~5% しか forward していなかったため
   - 旧 `LlmLinguaCompressor` (LLM 委譲版) を `LlmDelegationCompressor` に
     リネーム (実体は LLMLingua ではなく汎用 LLM 委譲圧縮で命名が
     misleading)。新 impl `LlmLingua2Compressor` が paper-faithful ONNX 版
   - benches/runner に `tier_0_1_2_compress` helper を追加し
     `TSUMUGI_LLMLINGUA2_MODEL_PATH` / `TSUMUGI_LLMLINGUA2_TOKENIZER_PATH`
     両方設定時は LlmLingua2、未設定は `truncate_compress` に fallback。
     LongMemEval / MemoryAgentBench / RULER の 3 adapter の tier-0-1-2
     経路をすべて切替
   - HF Hub にネイティブ ONNX 配布が無いため `download_llmlingua2.sh` が
     `optimum-cli export onnx --task token-classification` で都度 export、
     `~/.cache/tsumugi/llmlingua2-mbert/` に出力。`bench.yml` は
     `actions/cache@v4` で export 成果物を永続化し、`$GITHUB_ENV` 経由で
     パスを bench runner に流す
   - **CR/Oracle/RULER の圧縮率 vs recall 計測**: `bench.yml`
     workflow_dispatch (`--ablations tier-0-1-2`) で取得。本 PR では実機
     dispatch 未実施
   - **規模**: tsumugi-core +~400 行 / benches/runner +~50 行 /
     scripts +~70 行 / bench.yml +~25 行
3. **SetFit + MiniLM 実装** (`QueryClassifier`) — **完了 (2026-04-30)**
   - `tsumugi-core/src/classifier/setfit_classifier.rs` 新設、
     `SetFitClassifier` を `OnnxEmbedding` + `LinearHead` の compose で
     実装。inference path: query → ONNX session → 384-dim L2-normalized
     vector → matmul with linear head → bias add → argmax → label string
     → `QueryClass`。
   - **Head ファイル形式** (JSON、`LinearHeadFile` で公開):
     ```json
     {"labels": [...], "embedding_dim": 384, "weights": [[...], ...], "bias": [...]}
     ```
     downstream の Python `setfit` 訓練パイプラインが直接書き出せる。
   - **訓練データの責務分離**: `tsumugi-core` は推論 + JSON head loader
     のみ提供。per-class examples / contrastive fine-tune / linear head
     fit はすべてダウンストリーム責務。default ラベル
     `{Literal, Narrative, Analytical, Unknown}` への label string
     mapping は case-insensitive、未知ラベルは `with_default` で指定可
   - **多言語対応**: 既定の `OnnxEmbedding` (MiniLM-L6-v2 想定、英語のみ)
     を `with_embedder` で `paraphrase-multilingual-MiniLM-L12-v2`
     (~118M, 384-dim) 等に置換可能。head の `embedding_dim` は encoder
     の出力次元と一致させる必要がある。日本語クエリ運用ではこの swap
     を必須とし、本計画 § 7.2 のリスク表に従う
   - **bench wiring 不要**: 既存 `tsumugi-bench` は `QueryClassifier` を
     消費していない (ベンチは BM25 / HybridRetriever / compressor を直接
     呼ぶ)。ライブラリ API + 単体テスト + env-var-gated smoke の 3 点で
     完結
   - **規模**: tsumugi-core +~370 行 (impl + tests)、scripts なし、
     bench.yml 変更なし
4. **NliZeroShot 実装** (`EventDetector`) — **完了 (2026-04-30、GLiNER2 から
   方針変更)**

   **方針変更の根拠**: GLiNER2 採用は当初計画通りだったが、PR #35 後の
   着手前調査で以下の客観事実が判明した:

   - **F1 で DeBERTa が上**: GLiNER2 paper (arXiv:2507.18546) 自身が
     「Amazon-Intent / 20 Newsgroups の zero-shot 分類で DeBERTa-v3 が
     +5-6 pt 上回る」と報告。GLiNER2 の主張する優位性は latency のみ
     (20 label で 6.8× 速い)
   - **typical EventDetector の label 数 4-10 では latency 差が
     ボトルネックにならない**: DeBERTa の N-pass で 1-1.5s、Tier 2/3 の
     retrieval coast (10-50s) に対して dominant ではない
   - **GLiNER2 preprocessor は ~500 行の Rust 移植が必要**:
     `[ENT] label1 [ENT] label2 [SEP] tok1 ...` プロンプト構築、
     `words_mask` (subword → word index)、`span_idx` / `span_mask`
     (候補 span 全列挙)、`(B, L, K, C)` 出力 + greedy NMS decoder。
     gliner v1 は構造判明 (`urchade/gliner` で確認済) だが、
     **gliner2 Python パッケージの ONNX export 経路は公開 README に
     記載なし**。実 weights なしでは preprocessor の正しさを検証できず、
     「正確に動作してから次へ」方針と相性が悪い
   - **多言語が単一モデルで完結**: mDeBERTa-XNLI は 100+ 言語対応で
     エンコーダ事前学習に日本語含む。GLiNER2 は `-multi-v1` を別途
     採用する必要

   この事実から計画 § 7.4 のフォールバック (b) に降格し、
   `NliZeroShotDetector` (DeBERTa-MNLI / mDeBERTa-XNLI) を採用。

   **実装内容**:

   - `tsumugi-core/src/detector/nli_zero_shot.rs` 新設、
     `NliZeroShotDetector` を ort + tokenizers の標準パターンで実装
     (Phase 4-γ Step 1-3 と同じ lazy session init / `spawn_blocking` /
     `Mutex<Session>`)
   - 各 label について `(premise, hypothesis)` ペアで forward → softmax
     → P(entailment) → threshold 判定で `DetectedEvent` を発火
   - premise / hypothesis テンプレートをユーザ設定可
     (`{text}` / `{new_turn}` / `{label}` placeholder)。chunk + new_turn
     の結合方法、hypothesis の言い回しを domain 別にカスタマイズ可能
   - `with_threshold` (default 0.5)、`with_entailment_class_index`
     (default 0、mDeBERTa-XNLI 規約)、`with_max_sequence_length`
     (default 512) で運用調整
   - default モデル: `MoritzLaurer/mDeBERTa-v3-base-xnli-multilingual-nli-2mil7`
     (~278M, MIT、多言語)。英語専用なら `MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli`
     (~184M, MIT) に constructor 引数で差し替え
   - bench wiring 不要 (既存 `tsumugi-bench` は `EventDetector` を消費
     していない、Step 3 SetFit と同じ理由)
   - **規模**: tsumugi-core +~410 行 (impl + tests)、scripts なし、
     bench.yml 変更なし

5. **DistilBART 実装** (`Summarizer`) — **完了 (2026-04-30)**
   - `tsumugi-core/src/summarizer/distilbart/{mod.rs,inference.rs}` 新設、
     `DistilBartSummarizer` を `OnnxEmbedding` / `LlmLingua2Compressor`
     と同じ ort + tokenizers パターンで実装。Optimum の
     `text2text-generation-with-past` task export を直接食う 3 ONNX graph 構成
     (encoder / decoder / decoder_with_past) + greedy 生成 + KV cache reuse
   - `past_key_values.*` slot 名を `decoder_with_past.inputs` から動的に
     発見し layer 数に依存しない実装 (BART-large-CNN 12 層 / DistilBART
     6 層 どちらでも動作)
   - `min_output_length` で EOS 抑止、`max_output_length` で hard cutoff、
     special token IDs (BOS/EOS/PAD/decoder_start) は constructor で
     override 可
   - `SummaryMethod::DistilBart` enum variant を新設: `models/tsumugi/core.als`
     に `one sig DistilBart extends SummaryMethod {}` を追加し
     `scripts/regen.sh` で生成 Rust + 生成 TS を再生成、hand-written domain
     (`tsumugi-core/src/domain/summary.rs` と `tsumugi-ts/src/domain/summary.ts`)
     も同期
   - **bench wiring**: `benches/scripts/download_distilbart.sh`
     (`optimum-cli export onnx --task text2text-generation-with-past --model
     sshleifer/distilbart-cnn-6-6`) で 3 ONNX file + tokenizer.json を
     `~/.cache/tsumugi/distilbart-cnn-6-6/` に出力。`bench.yml` に
     `actions/cache@v4` (export 結果を永続化) + `$GITHUB_ENV` 経由の
     `TSUMUGI_DISTILBART_DIR` 配線。`DistilBartSummarizer::from_dir(dir)`
     で 4 file を一括ロード
   - bench runner 側の `Summarizer` 消費は無いので回路接続は不要 (Step 3
     SetFit / Step 4 NliZeroShot と同じパターン)
   - **規模**: tsumugi-core +~720 行 (impl + tests)、scripts +~85 行、
     bench.yml +~20 行

6. **GLiNER2 実装** (`EventDetector`、deferred Phase 4-β / 5)
   - 採用ブロッカー: gliner2 ONNX export 経路の公開、`gliner2`
     preprocessor の Rust 移植 (~500 行)、実 weights ベンチでの検証
   - **再検討トリガー**: NliZeroShot のレイテンシが運用上の bottleneck
     になった場合 (e.g. label 数が 20+ に増えた、Tier 2 latency 予算を
     越えた)。それまでは TODO に保留
...
### 5.3 既存 LLM 委譲版の扱い — **方針転換 (2026-04)**

当初計画では LLM 委譲版を「並列維持」として残す予定だった (本 §
末尾の「§ 7.5」参照) が、Phase 4-γ Step 5 まで encoder-only 実装が
全 5 trait で揃った時点で、tsumugi を **完全 encoder-only スタックに
一本化** する判断に変更した。

**実施した削除** (2026-04 の LLM 削除 PR):

- `traits/llm.rs` — `LLMProvider` / `CompletionRequest` / `GrammarSpec`
  / `ModelMetadata` を含む trait + 型一式
- `providers/mock_llm.rs` — `MockLLMProvider` (テスト用 fixture)
- `providers/stubs.rs` — `OpenAiCompatibleProvider`, `LmStudioEmbedding`
- `summarizer/llm.rs` — `LlmSummarizer`
- `compressor/llm_delegation.rs` — `LlmDelegationCompressor` (旧
  `LlmLinguaCompressor` を Phase 4-γ Step 2 でリネームしたもの)
- `classifier/bert_classifier.rs` — `BertClassifier` (LLM 委譲スタブ)
- `detector/llm_classifier.rs` — `LLMClassifierDetector`
- `SummaryMethod::LlmFull` enum variant (Alloy + 生成 + 手書き domain)
- `network` Cargo feature + reqwest 依存
- bench runner の `Suite::Health` / `Ablation::Full` / `run_*_full` 系
- CI scripts: `install_llama_cpp.sh` / `start_llama_server.sh` /
  `wait_for_health.sh`、`download_models.sh` の Qwen GGUF 部分

**転換の根拠** (cr ablation 実機 dispatch 結果、Phase 4-α Step 3 PR ③〜
Phase 4-γ Step 5 fix):

- LLM ablation (Qwen3.5-4B Q4_K_M @ 4 vCPU) で 1 case あたり 7-13 分。
  retrieval-only は数十 ms。**latency 差が約 4 桁**
- LLM 経路の accuracy 0/2 (cancel 前 2 件)、retrieval-only は 7/8 ~
  6/8。LLM が retrieved chunks から答えを抽出するシナリオが CPU では
  成立せず、回答長 3 token で打ち切られる症状を観測
- Phase 4-γ で landed した encoder-only 5 trait 実装が、本来 LLM
  委譲版を残す動機 (= 高品質オプション) を消去した

**結果**:

- tsumugi-core は autoregressive LLM API を呼ばない library として
  確定。下流製品が LLM を使う場合は、tsumugi の `CompiledContext` を
  製品自身の LLM クライアントへ流す
- `network` feature は撤去、HTTP 依存無しの crate に縮退
- bench は llama-server を立てる必要がなくなり、PR CI に組込み可能な
  軽量 job に整理 (`bench.yml` snapshot regression check、~5 min)
...
| MiniLM-L6-v2 | ❌ (英語) | multilingual-MiniLM-L12-v2 (~118M) または paraphrase-multilingual-MiniLM-L12-v2 への切替。`SetFitClassifier::with_embedder` で 1 行差し替え可 (Phase 4-γ Step 3 で公開) |
...
### 7.5 既存 LLM 委譲版の API 互換性 — **解決済 (2026-04)**

当初は「並列維持」を採用予定だったが、§ 5.3 で記録した通り 2026-04 に
方針転換し、LLM 委譲版を完全削除した。

互換性影響 (downstream consumer 視点):

- `tsumugi-core::traits::llm` モジュールが消滅。`LLMProvider` を
  消費するダウンストリームコードは tsumugi-core 経由ではなく自前の
  LLM クライアント実装に置き換える必要がある (tsumugi 経由の依存は
  もともと薄かったので、製品側の影響は限定的)
- `LlmSummarizer` / `LlmDelegationCompressor` / `BertClassifier` /
  `LLMClassifierDetector` を直接構築していたコードはコンパイル不可。
  対応する encoder-only 実装 (`DistilBartSummarizer` /
  `LlmLingua2Compressor` / `SetFitClassifier` / `NliZeroShotDetector`)
  に切替
- `SummaryMethod::LlmFull` を期待していた enum match は variant 不在
  でコンパイル不可。`SummaryMethod::DistilBart` 追加で代替

`tsumugi-cli` は LLM 経路を消費していなかったので影響なし。
