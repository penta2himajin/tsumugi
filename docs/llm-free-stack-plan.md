...
1. **`ort` 統合と `OnnxEmbedding` 実装**
   - `tsumugi-core` の Cargo features に `onnx` を追加 (既に skeleton あり)
   - `OnnxEmbedding::embed` の本実装 (multilingual-e5-small で動作検証)
   - benches/runner で `tier-0-1` の `MockEmbedding` を `OnnxEmbedding`
     に置換、CR/Oracle/RULER で再計測
   - **規模**: 1 PR、~300-500 行
