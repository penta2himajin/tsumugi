...
3. **LLM**: CI 内推論を主軸、API フォールバックを補助軸とする。**第 1 候補: `Qwen3.5-4B-Instruct` Q4_K_M (~3GB, Apache 2.0、262K context、201 言語、思考モード対応)** を `llama-server` (OpenAI 互換、最新ビルド必須) で起動し、既存 `OpenAiCompatibleProvider` をそのまま叩く。**安定 fallback: `Qwen3-4B-Instruct` (Apache 2.0、枯れた標準アーキテクチャ、~2.5GB)**。**第 3 候補: `Gemma 4 E4B` (Apache 2.0, 多言語 140+)**。43 ケース × 平均 200 出力トークン × 5–8 tok/s ≒ 15–30 分で完走する見込み。
...
|---|---|---|---|---|---|---|---|
| **Qwen3.5-4B-Instruct** | 5B 実測 (公称 4B) | **Apache 2.0** (確認済み) | **262K** native (1M YaRN) | ~3 GB | Hybrid Gated DeltaNet + MoE + Vision Encoder | 201 | **第 1 候補**: 性能・長文脈・多言語で優位、思考モードあり。VL モデルだが mmproj 不要なら text-only で動く。**llama.cpp 最新ビルド必須** |
| **Qwen3-4B-Instruct** | 4B | **Apache 2.0** | 32K | ~2.5 GB | 標準 Transformer | 119 | **安定 fallback**: 枯れた標準アーキテクチャ、CI 安定性最優先時 |
| **Gemma 4 E4B-it** | ~4B effective | **Apache 2.0** (2026-03 化) | 128K | ~3 GB | 標準 Transformer | 140+ | **第 3 候補**: 長文脈と多言語に強い、Apache 2.0 化が新しい |
| Qwen3.5-0.8B | ~0.6B | **Apache 2.0** (確認済み) | 262K | ~0.5 GB | Hybrid DeltaNet + Vision | 201 | **下振れ用**: 4B が間に合わない場合の代替 |
| Qwen3-1.7B-Instruct | 1.7B | **Apache 2.0** | 32K | ~1.1 GB | 標準 Transformer | 119 | 下振れ用: 標準アーキで安定 |
| Gemma 4 E2B-it | ~2B effective | **Apache 2.0** | 128K | ~1.5 GB | 標準 Transformer | 140+ | E4B が間に合わない場合 |
| Qwen3.5-2B | 2B | **Apache 2.0** (確認済み) | 262K | ~1.5 GB | Hybrid DeltaNet + Vision | 201 | **公式が「プロトタイピング/研究用途」と明示**、思考ループ報告あり。CI ベンチ主軸には不適 |
| Phi-4-Mini | 3.8B | MIT | 128K | ~2.5 GB | 標準 Transformer | 英語強 | 多言語が弱め、副次候補 |
| Qwen2.5-3B-Instruct | 3B | (Apache 2.0 / Qwen Research 混在) | 32K | ~1.9 GB | 標準 Transformer | 多言語 | Qwen2.5 系はライセンス境界要確認、Qwen3 系で十分なため棄却 |

### 選択

- **既定**: `Qwen3.5-4B-Instruct` Q4_K_M。HF モデルカードで Apache 2.0 を確認済み (2026-04-28 再調査)。Qwen3-4B (32K) と比べて 262K native context、201 言語、思考モード/非思考モード切替、性能ベンチで Qwen3-4B-2507 を上回る。**ただし注意点が 3 点**:
  1. **Multimodal VL モデル** (Image-Text-to-Text)。テキスト専用ベンチでは mmproj をロードしないことで text-only 動作可能 (llama.cpp で `--mmproj` フラグを付けない / vLLM では `--language-model-only`)。vision encoder 分のメモリオーバーヘッドはあるが運用上は無視できる。
  2. **Hybrid Gated DeltaNet + Gated Attention + sparse MoE** アーキテクチャ。GGUF 配布は存在するが、配布元のモデルカードが「Ensure you are using the absolute latest version of llama.cpp to support these new operators」と明記。**llama.cpp は最新ビルド (2026-04 以降の master 系) を pin する必要あり**、release バイナリの古いタグでは動かない可能性が高い。CPU での DeltaNet kernel 最適化状況は実測必須 (Phase 4-α Step 1 で smoke test を組む)。
  3. **5B params 実測** (公称 4B は LM の "active" 表記)。Q4_K_M GGUF で約 3 GB、Qwen3-4B (~2.5 GB) より重い。
- **安定 fallback**: `Qwen3-4B-Instruct` Q4_K_M (Unsloth Dynamic 推奨)。標準 Transformer アーキテクチャ、llama.cpp の release バイナリで確実に動く、CI 安定性最優先時はこちらに切替。`network` feature 既存配線がそのまま使える点は両者共通。
- **第 3 候補**: `Gemma 4 E4B-it` Q4_K_M。Apache 2.0 化済み、多言語 140+、長文脈 128K、標準アーキで CI 安定性が高い。多言語性能で Qwen3.5 と競合する場面で再評価。
- **下振れ用**:
  - `Qwen3.5-0.8B` (Hybrid DeltaNet、~0.5 GB)
  - `Qwen3-1.7B-Instruct` (標準 Transformer、~1.1 GB) — アーキテクチャ安定性重視ならこちら
- **棄却**: `Qwen3.5-2B` は Qwen 公式が「intended use cases are prototyping, task-specific fine-tuning, and other research or development purposes」と明示し、さらに「more prone to entering thinking loops compared to other Qwen3.5 models」とも記載。CI nightly の主軸には不適。
- **棄却**: Qwen2.5-3B は Qwen2.5 系列でライセンス境界が混在しており検証コスト過多。Qwen3 / Qwen3.5 系で十分。

### llama.cpp サーバーモード経由

- ジョブ内で `apt install` 経由ではなく、`ggml-org/llama.cpp` の release バイナリを GitHub Releases からダウンロード。**バージョン pin 必須**:
  - **Qwen3.5 (主候補) を使う場合**: 最新の master 系 build を pin (2026-04 以降の `b6000+` 系を想定、Step 1 の smoke test で具体的タグを確定する)。Hybrid Gated DeltaNet operator のサポートが新しいため、古い release では起動失敗の可能性が高い。
  - **Qwen3 (fallback) を使う場合**: 標準 Transformer なので `b4500` 等の安定タグで動く。
- 起動例 (Qwen3.5-4B、text-only モード、mmproj 非ロード):
  ...
  ```
- 起動例 (Qwen3-4B、fallback):
  ```bash
  llama-server -hf Qwen/Qwen3-4B-Instruct-GGUF:Q4_K_M --port 8080 --ctx-size 16384 --threads 4
  ```
- 既存 `OpenAiCompatibleProvider` から `http://localhost:8080/v1` を叩く。
- グラマー制約: `--grammar-file` で JSON 強制、回答抽出を安定化。
- **CPU 推論速度の実測がリスク**: Qwen3.5 の Hybrid DeltaNet kernel は llama.cpp の CPU パスでまだ最適化が進行中との第三者報告あり。Step 1 の smoke test で Qwen3.5-4B と Qwen3-4B の生成速度 (tok/s) を実測し、Qwen3.5 が著しく遅い場合は fallback に切替。
...
| Qwen3.5-4B | Apache 2.0 (HF model card 確認、2026-04-28) | 重みは HF Hub から download、Mayu に同梱しない | ✅ |
| Qwen3-4B (fallback) | Apache 2.0 | 同上 | ✅ |
