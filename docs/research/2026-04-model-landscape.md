...
このドキュメントは、tsumugi の MVP 準備段階 (2026-04-22) で行ったオープンモデル状況の調査内容を生データとして残したもの。後続の判断で見返す目的。
...
1. Qwen3 Swallow 8B Q5_K_M の日本語文体品質 (執筆系ユースケース)
2. Qwen3 Swallow 8B Q5_K_M の対話 / 役割演技口調一貫性 (TRPG / 対話系ユースケース)
3. Qwen2.5-Coder 7B Q5_K_M の DSL / 構造化出力品質 (コーディング / ゲームスクリプト系ユースケース)
4. Qwen3-30B-A3B Q4 で上位体験が 8B と有意差を生むか
5. MacBook Air M1/M2 16GB で Qwen3 Swallow 8B が快適に動くか (最低構成ライン)
6. Apple MLX 経由での Gemma 4 / Qwen 3.5 の対応状況 (Mac ユーザー向け)
7. GBNF 制約下での Qwen2.5-Coder 7B の JSON + JavaScript 生成精度
