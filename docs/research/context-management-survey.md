...
| EventDetector cascade (chatstream 流) | LLM 呼び出しを最小化、既に設計済 | 済 | Phase 1 採用済 |
...
| MACLA Bayesian selection | 手続き記憶向き、創作/ツクールでの適合性要検証 | 未計画 |
| Acon Context Optimization | Agent 系、つむぎへの直接マッピング要検討 | 未計画 |
| GAM 役割中心文脈化 | creative feature 内で Character と統合する余地 | 保留 |
