...
このドキュメントは、tsumugi の設計方針を既存研究と照らし合わせるために 2026-04-22 に行った調査の記録。つかさ/つづり/つくもの MVP 実装を控え、「長期プロジェクトの動的コンテキスト管理」の既存手法を整理し、tsumugi に取り込むべき要素を抽出する目的。
...
| D. 構造化抽出 | Mem0 facts, Zep entities | Fact / LoreEntry 設計済 |
| E. 階層的要約 | **RAPTOR** | **未採用、検討候補** |
| F. 自己編集メモリ | MemGPT, Letta | 未採用、tsumugi 哲学との整合性要検討 |
| G. 役割中心文脈化 | GAM (2026) | 部分的 (Character 抽象) |
| H. 話題切替圧縮 | chatstream 3 段カスケード | EventDetector で設計済 |
| I. RL によるメモリ最適化 | MemAgent (2025-07), MemRL | 見送り (実装コスト高) |
...
**SelRoute (2026)** — tsumugi のニーズに最も近い:
...
**LLMLingua-2 (ACL 2024)** — プロンプト圧縮を token 分類問題として定式化:
...
**IKE: Isolation Kernel Embedding (2026-01)**:
...
**DAM: Decision-theoretic Agent Memory (2025-12)**:
...
tsumugi の既存 Chunk 構造は親子関係を持つが、要約は Chunk 単位で 1 つ。RAPTOR を取り込むには:

1. 親 Chunk が子 Chunk 群の**再帰的要約**として機能するよう意味付けを明示化
2. 階層レベルの明示 (Raw / Scene / Chapter / Arc / Project)
3. 階層間の要約伝播タイミング (差分更新)
4. ユーザー編集済み要約の保護 (locked フラグ)

### 既存 Chunk 構造で表現する代替案

新たな `HierarchicalSummary` 型を追加せず、既存の Chunk に以下メタデータを拡張する案:

```rust
pub struct Chunk {
    // 既存フィールド...
    pub summary_level: SummaryLevel,         // Raw / Scene / Chapter / Arc / Project
    pub summary_method: SummaryMethod,       // LLM / LLMLingua2 / Extractive / UserManual
    pub edited_by_user: bool,                // ユーザー編集済み
    pub auto_update_locked: bool,            // 自動更新を防ぐフラグ
}
```

親子関係は既存の parent/children で表現、要約は既存の summary フィールド。
...
| 手法 | 採用理由 | 実装コスト |
|---|---|---|
| SelRoute 流クエリルーティング | 正規表現 + 小型 BERT、GPU 不要、最小構成で動く | 低 |
| SQLite FTS5 + lindera BM25 | 既に計画済み、最小構成基盤 | 済 |
| MiniLM / BGE-small 埋め込み | CPU 動作、第一選択 | 低 |
| EventDetector cascade (chatstream 流) | LLM 呼び出しを最小化、既に設計済 | 済 |

### 6.2 推奨 (Phase 1 後半〜 Phase 2)

| 手法 | 採用理由 | 実装コスト |
|---|---|---|
| LLMLingua-2 | Context Compiler の圧縮段、3-6x 高速 | 中 |
| IKE 二値 embedding | 最小構成で高速類似検索 | 中 |
| fastc / ModernBERT 分類器 | Fact 抽出 / pending item 検出の軽量化 | 中 |
| 階層的要約 (RAPTOR 流) | 既存 Chunk 構造の拡張で表現可 | 中 |
| HierarchicalSummary 編集 UI | ユーザー調整可能性 (Kenya の要望) | 中 |

### 6.3 興味深いが実験段階 (将来検討)

| 手法 | コメント |
|---|---|
| DAM Decision-theoretic Framework | 枠組み成熟待ち |
| MACLA Bayesian selection | 手続き記憶向き、創作/ツクールでの適合性要検証 |
| Acon Context Optimization | Agent 系、つむぎへの直接マッピング要検討 |
| GAM 役割中心文脈化 | creative feature 内で Character と統合する余地 |
...
## 7. tsumugi の処理階層 (4 tier) の提案

Kenya のニーズ (高速かつ効率的、LLM 非依存) を整理すると、以下の階層が自然:

### Tier 0: ゼロコスト (即応、数 μs 〜 数 ms)

- 正規表現ルール (SelRoute 流)
- 完全一致キーワード (chatstream Stage 1)
...
- 軽量 LLM による yes/no 判定 (chatstream Stage 3)
...
現状の EventDetector cascade は既にこの 4 層構造を持つ。他のコンポーネントにも同じ階層を適用する余地:

- **Retriever**: 現状は Tier 0-1 のみ (LLM 不要)
- **Context Compiler**: Tier 0-1 で大半を処理、Tier 3 は最終生成のみ
- **Summarizer (Phase 2)**: Tier 2 (LLMLingua-2) と Tier 3 (LLM) の使い分け
- **Fact 抽出**: Tier 1 (BERT 分類器) で候補抽出、Tier 3 (LLM) で最終整形

## 8. 設計影響のメモ (後日 tech-architecture.md に反映する際の材料)
...
pub trait PromptCompressor: Send + Sync {
    ...
    async fn compress(&self, text: &str, ratio: f32) -> Result<String>;
}
...
pub trait Summarizer: Send + Sync {
    // 階層的要約生成、method 指定可
    async fn summarize(
        &self, chunks: &[Chunk], method: SummaryMethod
    ) -> Result<String>;
}
...
pub enum SummaryLevel {
    Raw,        // 元のチャンク
    Scene,      // シーン単位 (2-5 chunks)
    Chapter,    // 章単位
    Arc,        // ストーリーアーク / プロジェクトフェーズ
    Project,    // プロジェクト全体
}
```
...
- **Phase 1**: InMemoryQueryClassifier (正規表現), HybridRetriever (BM25 + cosine), 既存 Chunk に summary_level / edited_by_user 追加
- **Phase 2**: BertQueryClassifier (MiniLM / ModernBERT), LlmLinguaCompressor, HierarchicalSummarizer
- **Phase 3**: IkeEmbeddingProvider (二値化), Decision-theoretic experiments

## 9. tsumugi の独自性 (既存研究と比較)

整理を通じて、既存の memory framework 群と tsumugi の差分が明確になった:

- **ドメイン特化層を feature flag で分離**: Mem0 / Zep / MemGPT は汎用チャット前提、tsumugi は creative / その他拡張を明示
...
- **LLM 非依存の tier 構造**: Kenya の「非効率性回避」哲学を設計に取り込む
...
- 階層的要約の導入を既存 Chunk 拡張で行うか、新規 HierarchicalSummary 型にするか
- LLM 自己編集メモリの是非 (制御性 vs 自律性のトレードオフ)
- MemoRAG 流のグローバル要約 (本体別途取得の 2 段階) の実装コスト
- Event-Centric Memory (MAGMA, EverMemOS) のグラフ構造と Chunk 構造の両立
- PromptCompressor / QueryClassifier / Summarizer trait の追加タイミング
